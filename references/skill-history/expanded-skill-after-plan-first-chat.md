# Expanded Skill Snapshot After Plan-First Chat Method

This snapshot preserves the current generic skill after incorporating the chat-history method: save a bounded plan markdown before substantive manuscript edits, then execute only that plan.
...
## Added Conceptual Layer

The expanded version adds:

- Plan markdown before substantive edits.
- Stable active file names with preserved baselines.
- Explicit latest, original, and redlined file roles.
- Stronger rule that execution must stay within the accepted plan.
- Generic publishable wording without project-specific paper details.
